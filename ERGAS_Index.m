function ERGAS_ind = ERGAS_Index(x, MSWV_US, W_R, W_G, W_B, W_NIR, F_P, F_R, F_G, F_B, F_NIR, P, I, bandCoeffs, MS_OBJ)
...
% F_PSO(:,:,1) = MSWV_US(:,:,1) + W_R.*(x(1)*F_P+(1-x(1))*F_R).*(P-I);
% F_PSO(:,:,2) = MSWV_US(:,:,2) + W_G.*(x(2)*F_P+(1-x(2))*F_G).*(P-I);
% F_PSO(:,:,3) = MSWV_US(:,:,3) + W_B.*(x(3)*F_P+(1-x(3))*F_B).*(P-I);
% F_PSO(:,:,4) = MSWV_US(:,:,4) + W_NIR.*(x(4)*F_P+(1-x(4))*F_NIR).*(P-I);
